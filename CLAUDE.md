...
| Day | Task | Gate |
|---|---|---|
| 1 | Scaffold: Next.js + FastAPI + Docker Compose | GET /health → 200 all services |
| 2 | Supabase Auth + JWT middleware | Register → valid JWT → 401 without token |
| 3 | MongoDB Atlas: users, quests, progress collections | Profile page loads from MongoDB |
| 4 | Groq API integration + AI Adapter + prompt template | /ai/generate → 3 valid quest objects |
| 5 | Quest Service: /quests/generate end-to-end | Real user context → quests stored in MongoDB |
| 6 | Progress Service: completion + XP calculation | Quest marked done → XP stored |
| 7 | Next.js dashboard: quests + XP + streak display | User sees 3 quests, marks done, XP updates |
| 8 | XP + Level formulas in Gamification Engine | Level derived from XP, bar shows correct % |
| 9 | Streak Engine: Redis streak counters | Streak increments daily, resets on miss |
| 10 | E2E test + Railway + Vercel deploy | Full loop on public URL in < 5 minutes |
